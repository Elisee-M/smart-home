...
const HistoryChart = () => {
  ...
  const loadHistoryData = async () => {
    try {
      ...
      const dhtHistory = await firebaseUtils.readOnce('history/DHT');

      const combinedData = new Map<number, CombinedHistoryData>();

      // Process DHT data
      if (dhtHistory) {
        Object.entries(dhtHistory).forEach(([key, item]: [string, any]) => {
          const { formattedTime, formattedDate } = formatTimestamp(item.timestamp);
  ...
  const handleDeleteHistory = async () => {
    try {
      setDeleting(true);
      await firebaseUtils.write('history/DHT', null);
      setHistoryData([]);
      toast({
        title: "Success",
  ...
  useEffect(() => {
    ...
    const dhtHistoryRef = firebaseUtils.subscribe('history/DHT', () => {
      loadHistoryData(); // Reload all data when DHT history changes
    });
                ...
                <>
                  <div className="bg-orange-500/10 p-3 sm:p-4 rounded-lg border border-orange-200/30">
                    ...
                    <p className="text-xs sm:text-sm text-muted-foreground mt-1">
                      {(() => {
                        const tempData = historyData.filter(d => d.Temperature !== undefined);
                        const avg = tempData.length > 0 
                          ? tempData.reduce((sum, d) => sum + (d.Temperature || 0), 0) / tempData.length 
                          : 0;
                  ...
                  <div className="bg-red-500/10 p-3 sm:p-4 rounded-lg border border-red-200/30">
                    ...
                    <p className="text-xs sm:text-sm text-muted-foreground mt-1">
                      {(() => {
                        const tempData = historyData.filter(d => d.Temperature !== undefined).map(d => d.Temperature || 0);
                        const max = tempData.length > 0 ? Math.max(...tempData) : 0;
                        return isFinite(max) ? max.toFixed(1) : '0.0';
                      })()}°C
                    </p>
                  </div>
                </>
              ...
              {historyData.some(d => d.Humidity !== undefined) && (
                <>
                  <div className="bg-blue-500/10 p-3 sm:p-4 rounded-lg border border-blue-200/30">
                    ...
                    <p className="text-xs sm:text-sm text-muted-foreground mt-1">
                      {(() => {
                        const humidityData = historyData.filter(d => d.Humidity !== undefined);
                        const avg = humidityData.length > 0 
                          ? humidityData.reduce((sum, d) => sum + (d.Humidity || 0), 0) / humidityData.length 
                          : 0;
                        return isFinite(avg) ? avg.toFixed(1) : '0.0';
                      })()}%
                    </p>
                  ...
                  <div className="bg-cyan-500/10 p-3 sm:p-4 rounded-lg border border-cyan-200/30">
                    ...
                    <p className="text-xs sm:text-sm text-muted-foreground mt-1">
                      {(() => {
                        const humidityData = historyData.filter(d => d.Humidity !== undefined).map(d => d.Humidity || 0);
                        const max = humidityData.length > 0 ? Math.max(...humidityData) : 0;
                        return isFinite(max) ? max.toFixed(1) : '0.0';
                      })()}%
                    </p>
                  </div>
